...
export class UploadService {
  ...
  async presignedUrlS3(userId: string, presignedUrlDto: PresignedUrlDto): Promise<IPreSignUrl> {
    try {
  ...
  async presignedUrlS3Public(presignedUrlDto: PresignedUrlDto): Promise<IPreSignUrl> {
    try {
      const { fileName, fileType, pathType } = presignedUrlDto;
      const newFileName = getFileName(fileName);
